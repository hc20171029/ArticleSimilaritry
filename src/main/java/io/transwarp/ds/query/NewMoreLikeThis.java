 ...
 * </pre>
 */
public final class NewMoreLikeThis {
    ...
    private PriorityQueue<ScoreTerm> scoreTermsQueue=null;
    ...
    public PriorityQueue<ScoreTerm> getScoreTermsQueue(){
     ...
     */
    public NewMoreLikeThis(IndexReader ir) {
        this(ir, new ClassicSimilarity());
    }

    public NewMoreLikeThis(IndexReader ir, TFIDFSimilarity sim) {
        this.ir = ir;
        this.similarity = sim;
    }
    ...
    public Query like(String fieldName, Reader... readers) throws IOException {
        ...
        PriorityQueue<ScoreTerm> scoreTerms=createQueue(perFieldTermFrequencies);

        return createQuery(scoreTerms);
    }

    private void cloneScoreTermQueue(PriorityQueue<ScoreTerm> scoreTerms){
        scoreTermsQueue=new PriorityQueue<ScoreTerm>(scoreTerms.size()) {
            @Override
            protected boolean lessThan(ScoreTerm a, ScoreTerm b) {
                return false;
            }
        };
        Iterator<ScoreTerm> it=scoreTerms.iterator();
        while(it.hasNext()){
            scoreTermsQueue.add(it.next());
        }
    }
    ...
    private Query createQuery(PriorityQueue<ScoreTerm> q) {
        ...
        while ((scoreTerm = q.pop()) != null) {
            Query qq=null;
            if(!useSentenceFuzzy) {
                qq = new TermQuery(new Term(scoreTerm.topField, scoreTerm.word));
            }
            else{
                qq= new FuzzyQuery(new Term(scoreTerm.topField,scoreTerm.word),1);
            }
            if (boost) {
                if (bestScore == -1) {
                ...
                qq = new BoostQuery(qq, boostFactor * myScore / bestScore);
            }

            try {
                query.add(qq, BooleanClause.Occur.SHOULD);
            }
    ...
    private PriorityQueue<ScoreTerm> createQueue(Map<String, Map<String, Int>> perFieldTermFrequencies) throws IOException {
        ...
        for (Map.Entry<String, Map<String, Int>> entry : perFieldTermFrequencies.entrySet()) {
            ...
            for (Map.Entry<String, Int> tfEntry : perWordTermFrequencies.entrySet()) {
                String sentence = tfEntry.getKey();
                if(!SentenceUtil.isValidSentence(sentence)){
                    continue;
                }//对无效的句子（太短，实词太少的句子过滤掉）
                int tf = tfEntry.getValue().x;
                if (minTermFreq > 0 && tf < minTermFreq) {
                    continue;
                }

                int docFreq = ir.docFreq(new Term(fieldName, sentence));
                ...
                if (queue.size() < limit) {
                    ...
                    queue.add(new ScoreTerm(sentence, fieldName, score, idf, docFreq, tf));
                } else {
                    ScoreTerm term = queue.top();
                    if (term.score < score) {
                        term.update(sentence, fieldName, score, idf, docFreq, tf);
                        queue.updateTop();
                    }
                }
            }
